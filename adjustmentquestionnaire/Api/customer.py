from datetime import datetime,timedelta
import math

from django.db.transaction import atomic
from django.db.models import Q

from Api.rest import Rest
from myApp.models import *
from Api.utils import *
from Api.decorators import *
from  adjustmentquestionnaire.settings import DOMAIN

#客户问卷
class CustomerQuestionnaire(Rest):
    #创建问卷
    @atomic
    @customer_required
    def put(self,request,*args,**kwargs):
        user = request.user
        #获取请求数据
        data = request.PUT

        if user.is_authenticated:
                #创建问卷对象
                questionnaire = Questionnaire()
                #标题
                questionnaire.title = data.get('title', '')
                # 创建时间,特殊处理，创建时间使用当前时间
                questionnaire.create_date = datetime.now()
                # 截止时间，格式是：YYY - MM - DD，例如2010 - 10 - 20#截止时间，格式是：YYY - MM - DD，例如2010 - 10 - 20
                #截止时间截止时间,特殊处理“%Y-%m-%d”[int(item) for item  in  '2017-10-10'.split('-')]
                try:
                    deadline = datetime.strptime(data.get('deadline', ''), '%Y-%m-%d')
                except Exception as e:
                    #如果获取时间失败，那么使用当前时间加上10天
                    deadline = datetime.now() + timedelta(days=10)

                questionnaire.deadline = deadline
                #剩余数量，默认问卷数量为1份
                questionnaire.amount = int(data.get('amount',1))
                #问卷状态,特殊处理 问卷创建时,状态为草稿
                questionnaire.state = 0
                #问卷关联客户
                questionnaire.customer = user.customer
                questionnaire.save()
                return json_response({
                    "问卷的id：": questionnaire.id
                })

    #更新问卷
    @atomic
    @customer_required
    def post(self,request,*args,**kwargs):
            user = request.user
            data = request.POST
            #问卷id
            questionnaire_id= int(data.get('id',''))
            try:
                #找到问卷
                questionnaire = Questionnaire.objects.filter(id=questionnaire_id).first()
            except Exception as e:
                return params_error({
                    'msg': "找不到对应的问卷"
                })
            #判断是否属于当前用户
            if questionnaire.customer == user.customer:
                #问卷状态：0-草稿，1-待审核，2-审核失败，3-审核通过，4-已发布
                #判断问卷是否可以修改，0,2,3可以修改
                if questionnaire.state != 1:
                    # 创建问卷对象
                    questionnaire = Questionnaire()
                    #id
                    questionnaire.id = data.get('id', '')
                    # 标题
                    questionnaire.title = data.get('title', '')
                    try:
                        # 截止时间截止时间,特殊处理“%Y-%m-%d”
                        deadline = datetime.strptime(data.get('deadline', ''), '%Y-%m-%d')
                    except Exception as e:
                        deadline = datetime.now() + timedelta(days=10)
                    questionnaire.deadline = deadline

                    # 创建时间
                    questionnaire.create_date = data.get('create_date', '')
                    # 剩余数量
                    questionnaire.amount = int(data.get('amount', ''))
                    # 问卷状态
                    state = int(data.get('state', 0))
                    if state not in [0, 1]:
                        return params_error({
                            'state': '状态不合法'
                        })
                    # 特殊state
                    questionnaire.state = state
                    # 问卷关联
                    # 客户
                    questionnaire.customer = user.customer

                    questionnaire.save()
                    return json_response({"更新问卷成功，问卷的id：": questionnaire.id})
            else:
                return json_response({"msg:": "找不到对应的问卷,或者问卷不可修改"})

    #删除问卷
    @customer_required
    def delete(self,request,*args,**kwargs):
            user = request.user
            data = request.DELETE
            # 问卷ids
            ids = data.get('ids', [])
            #找到问卷
            questionnaires = Questionnaire.objects.filter(id__in=ids)
            ids = []
            for questionnaire in questionnaires:
                # 判断是否属于当前用户
                 if questionnaire.customer == user.customer:
                    #是否可删除，状态为0,2,3可删除
                    if questionnaire.state != 1:
                        # 把满足条件的问卷id取出来
                        ids.append(questionnaire.id)
                        #把满足条件的问卷删除
                        questionnaire.delete()
                        #把删除的问卷id返回给客户端
                    else:
                        return json_response({"msg": "问卷正在审核中无法进行删除！"})
            return json_response({"deleted_ids":ids})

    #获取问卷
    #@customer_login
    @customer_required
    def get(self,request,*args,**kwargs):
        user = request.user
        data = request.GET
        #localhost:8000/api/v1/customer_questionnaire?page=1&limit=10&status=0&with_detail=False&id
        #print(data['page'])
        state = data.get('state',False)
        limit = abs(int(data.get('limit',15)))
        start_id = data.get('start_id',False)
        title = data.get('title',False)
        create_date = data.get('create_date',False)
        with_detail = data.get('with_detail',False)
        page = abs(int(data.get('page',1)))
        Qs = []
        if state:
            state = [int(state)]
        else:
            state = [0,1,2,3,4]
        Qs.append(Q(state__in = state))
        if start_id:
            start_id = int(start_id)
        else:
            start_id =0
        Qs.append(Q(id__gt=start_id))
        if title:
            Qs.append(Q(title__contains=title))

        if create_date:
            create_date = datetime.strftime(create_date,'%Y-%m-%d')
            Qs.append(Q(create_date__gt = create_date))
        Qs.append((Q(customer = request.user.customer)))
        if limit >50:
            limit = 50
        all_objs = Questionnaire.objects.filter(*Qs)
        pages = math.ceil(all_objs.count()/limit)
        if page > pages:
            page = pages
        start = (page-1)*limit
        end = page*limit
        objs = all_objs[start:end]

        data = []
        for obj in objs:
            #构建单个问卷信息
            obj_dict = dict()
            obj_dict['id'] = obj.id
            obj_dict['create_date'] = datetime.strftime(
                obj.create_date, "%Y-%m-%d")
            obj_dict['deadline'] = datetime.strftime(obj.deadline, "%Y-%m-%d")
            obj_dict['state'] = obj.state
            obj_dict['amount'] = obj.amount
            if with_detail in ['true',True]:
                #构建问卷下的问题
                obj_dict['questions'] = []
                for question in obj.question_set.all().order_by('index'):
                    #构建单个问题
                    question_dict = dict()
                    question_dict['id'] = question.id
                    question_dict[title] = question.title
                    question_dict['category'] = question.category
                    question_dict['index'] = question.index
                    #构建问题选项
                    question_dict['item'] = [
                        {
                            "id":item.id,
                            "content":item.content,
                            "index":item.index
                        }for item in question.questionitem_set.all()
                    ]
                    #将问题添加到问卷的问卷的问题列表中
                    obj_dict['questions'].append(question_dict)
                    # 将问卷添加到问卷列表中
                    data.append(obj_dict)

                return json_response(
                    {
                        'page':pages,
                        'objs':data
                    }
                )

#问题接口
class CustomerQuestion(Rest):
    # 创建问题
    @atomic
    @customer_required
    def put(self,request,*args,**kwargs):
        data = request.PUT
        questionnaire_id = data.get('questionnaire_id',0)
        questionnaire_exists = Questionnaire.objects.filter(id=questionnaire_id,customer=request.user.customer,state__in=[0,2,3])
        if questionnaire_exists:
            questionnaire = questionnaire_exists[0]
        else:
            return params_error({
                'msg':'找不到问卷,或者问卷不可修改'
            })
        #添加问题
        question = Question()
        question.questionnaire = questionnaire
        question.title = data.get('title','题纲')
        question.category = data.get('category','radio')
        question.index  = int(data.get('index',0))
        question.save()
        #修改问卷状态
        questionnaire.state = 0
        questionnaire.save()
        #添加问题选项
        items = data.get('item',[])
        for item in items:
            question_item = QuestionItem()
            question_item.question = question
            question_item.content = item.get('content','')
            question_item.save()

        return json_response({
            'id':question.id
        })
    #更新问题
    @atomic
    @customer_required
    def post(self,request,*args,**kwargs):
        date = request.POST
        question_id = date.get('id',0)
        #判断需要修改的问题是否存在
        question_exits = Question.objects.filter(id = question_id,questionnaire__state__in=[0,2,3],
                                                 questionnaire_customer=request.user.customer)
        if not question_exits:
            return params_error({
                "msg":"该问题找不到,或者该问题所在问卷无法修改"
            })
        #更新问题属性
        question = question_exits[0]
        question.title = date.get('title','题纲')
        question.category = date.get('category','radio')
        question.index = int(date.get('index',0))
        question.save()
        #更新问题所在问卷的状态
        questionnaire = question.questionnaire
        questionnaire.state = 0
        questionnaire.save()

        items = date.get('items',[])
        question.questionitem_set.all().delete()
        for item in items:
            question_item = QuestionItem()
            question_item.question = question
            question_item.content = item.get('content','')
            question_item.save()
        return  json_response({
            'msg':'更新成功'
        })
    @atomic
    @customer_required
    def delete(self,request,*args,**kwargs):
        data = request.DELETE
        ids = data.get('ids',[])
        objs = Question.objects.filter(id__in = ids,questionnaire__state__in=[
         0,2,3],questionnaire__customer=request.user.customer)
        deleted_ids = [obj.id for obj in objs]
        questionnaie_set = set()
        for obj in objs:
            questionnaie_set.add(obj.questionnaire)
        for questionnaire in questionnaie_set:
            questionnaire.state = 0
            questionnaire.save()
        objs.delete()
        return json_response({
            'deleted_ids': deleted_ids
        })

#问卷状态
class CustomerQuestionnaireState(Rest):
    @atomic
    @customer_required
    def post(self,request,*args,**kwargs):
        date = request.POST
        questionnaire = Questionnaire()
        questionnaire_exits = questionnaire.objects.filter(id = date.get['id'],customer=request.user.customer,state__in=[0,3])
        if not questionnaire_exits:
            return json_response({
                'msg':'问卷不存在或者问卷无法进行提交'
            })
        else:
            if questionnaire_exits[0].state==0:
                questionnaire.state = 1
            elif questionnaire_exits[0].state==3:
                questionnaire.state = 4
            return json_response({
                    "msg": "修改成功"
                })

#问题题目标号接口
class CustomerQuestionIndex(Rest):
    #修改题目标号
    @atomic
    @customer_required
    def post(self,request,*args,**kwargs):
        data = request.POST
        questionnaire_exist = Questionnaire.objects.filter(id=data.get(
         "questionnaire_id",0),state__in=[0,2,3],customer=request.user.customer)
        if not questionnaire_exist:
            return params_error(
                {'questionnaire_id':'问卷不存在或者问卷不可更改'}
            )
        questionnaire = questionnaire_exist[0]
        question_exist = Question.objects.filter(questionnaire=questionnaire,id=data.get('id',0))
        if not  question_exist:
            return params_error(
                {'question_id':'问题不存在或者问题不可修改'}
            )
        question = question_exist[0]
        index = int(data.get('index',0))
        questionnaire.state = 0
        questionnaire.save()
        question.index = index
        question.save()
        return json_response({
            'msg':'更新成功'
        })

#钱包接口
class Wallet(Rest):
    #获取钱包信息
    @customer_required
    def get(self,request,*args,**kwargs):
        #找到当前客户的钱包
        wallet = Wallet.objects.get(customer__user=request.user)
        result = {
            "balance":wallet.balance
        }
        return json_response(result)
    #钱包充值,通过获取充值二维码的方式
    @customer_required
    @atomic
    def put(self,request,*args,**kwargs):
        data = request.PUT
        try:
            amount = abs(int(data.get('amount',0)))
            if amount==0:
                raise Exception('钱包金额不合法')
        except Exception:
            return params_error({
                "amount":"金额不合法"
            })
        payment = data.get('payment','zhifubao')
        #保存支付记录
        flow = WalletFlow()
        flow.amount = amount
        flow.payment = payment
        flow.wallet = request.user.customer.wallet
        flow.done = False
        flow.save()
        qrcode = create_qrcode(
            '{domin}/api/v1/paymentback?/amount={amount}&payment={payment}&flow_id={flow_id}'.format(domin=DOMAIN,amount=amount,payment=payment,flow_id=flow.id)
        )
        return json_response({
            "qrcode":qrcode
        })

#钱包充值支付记录接口
class WalletFlow(Rest):
    # 获取钱包充值支付记录
    @customer_required
    def get(self,request,*args,**kwargs):
        data = request.GET
        direction = int(data.get('direction',0))
        limit = abs(int(data.get('limit',15)))
        start_id = data.get('start_id',False)
        create_date = data.get('create_date',False)
        page = abs(int(data.get('page',1)))

        Qs = [Q(wallet__customer__user = request.user),Q(done=True)]
        if direction == 0:
            direction = False
        else:
            direction = True
        Qs.append(Q(direction=direction))

        if start_id:
            start_id = int(start_id)
        else:
            start_id = 0
        Qs.append(Q(id__gt=start_id))

        if create_date:
            create_date = datetime.strftime(create_date,'%Y-%m-%d')
            Qs.append(Q(datetime__gt=create_date))

        if limit > 50:
            limit = 50
        all_objs = WalletFlow.objects.filter(*Qs)
        pages = math.ceil(all_objs.count()/limit) or 1
        if page > pages:
            page = pages
        start = (page-1)*limit
        end = page*limit
        objs = all_objs[start:end]

        result = {
            "pages":pages,
            "objs":[{
                "id":obj.id,
                "amount":obj.id,
                "reason":obj.reason,
                "create_date":datetime.strftime(obj.create_date,'%Y-%m-%d %H:%M')
            }for obj in objs]
        }
        return json_response(result)

